...
public class FMRadioPlayerService extends Service {
	...
	protected IFMRadioServiceCallback mCallback = new IFMRadioServiceCallback.Stub() {
		...
		public void onCommandComplete(int cmd, int status, String value)
			...
			Log.v(TAG, "Got radio service event: cmd " + cmd + " status "
					+ status + " value " + value);

			switch (cmd) {
			case 0: {
				Message msg = Message.obtain(mHandler, MSG_TUNE_COMPLETE,
						status, Integer.parseInt(value), null);
				mHandler.sendMessage(msg);
				break;
			}
			case 1: {
				Message msg = Message.obtain(mHandler, MSG_SEEK_COMPLETE,
						status, Integer.parseInt(value), null);
				mHandler.sendMessage(msg);
				break;
			}
			case 2: {
				Message msg = Message.obtain(mHandler, MSG_SCAN_COMPLETE,
						status, 0, null);
				mHandler.sendMessage(msg);
				break;
			}
			...
			case 4: {
				Message msg = Message
						.obtain(mHandler, MSG_RDS_PS_UPDATE, value);
				mHandler.sendMessage(msg);
			}
				break;
			case 5: {
				Message msg = Message
						.obtain(mHandler, MSG_RDS_RT_UPDATE, value);
				mHandler.sendMessage(msg);
			}
				break;
			case 6:
				if (mUSBand) {
					...
					Message msg = Message.obtain(mHandler, MSG_RDS_PS_UPDATE,
							stationName);
					mHandler.sendMessage(msg);
				}
				break;
			case 7: {
				...
				Message msg = Message.obtain(mHandler, MSG_RDS_PTY_UPDATE,
						newPty, 0, null);
				mHandler.sendMessage(msg);
			}
				break;
			...
			case 15: {
				...
				if (mReady
						|| setSeekSensitivity(Preferences
								.getSeekSensitivityThreshold(context))) {
					break;
				}
				/*
	...
	private final IFMRadioPlayerService.Stub mBinder = new IFMRadioPlayerService.Stub() {
		...
		public boolean powerOn() {
			...
			if (isAirplaneModeOn()
					&& !Preferences
							.isAirplaneModeIgnored(FMRadioPlayerService.this)) {
			...
			int headsetState = headsetIntent != null ? headsetIntent
			...
			if (!mBound) {
				mBound = bindService(new Intent(
						"com.motorola.android.fmradio.FMRADIO_SERVICE"),
						mConnection, 1);
				if (!mBound) {
		...
		public boolean seek(int freq, boolean upward) {
			Log.d(TAG, "Got seek request, frequency " + freq + " upward "
					+ upward);
			if (mReady) {
				Message msg = Message.obtain(mHandler, MSG_SEEK_CHANNEL,
						upward ? 0 : 1, 0, null);
				mHandler.sendMessage(msg);
			}
			return mReady;
		...
		public void setAudioRouting(int routing) {
			...
			Message msg = Message.obtain(mHandler, MSG_SET_ROUTING, routing, 0,
					null);
			mHandler.sendMessage(msg);
		}
	...
	private final Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case MSG_SEEK_COMPLETE:
				...
				mCurFreq = msg.arg2;
				Log.v(TAG, "Seek completed, success " + (msg.arg1 != 0)
						+ " frequency " + mCurFreq);
				resetRDSData();
			...
			case MSG_SET_ROUTING:
				if (msg.arg1 == FM_ROUTING_HEADSET
						|| msg.arg1 == FM_ROUTING_SPEAKER) {
	...
	private void registerObserver() {
		...
		getContentResolver().registerContentObserver(Channels.CONTENT_URI,
				true, mObserver);
	}
		...
		mReceiver = new BroadcastReceiver() {
			...
			public void onReceive(Context context, Intent intent) {
				...
				if (action.equals(Intent.ACTION_HEADSET_PLUG)) {
					...
					int state = intent.getIntExtra("state", 0);
					Log.v(TAG, "Got airplane mode change message, new state "
							+ state);
					if (state != 0
							&& !Preferences.isAirplaneModeIgnored(context)) {
						FMUtil.showNoticeDialog(context,
								R.string.fmradio_airplane_mode_enabled);
						mHandler.sendEmptyMessage(MSG_SHUTDOWN);
					}
					...
					if (isMusicPlaying()) {
						FMUtil.showNoticeDialog(context,
								R.string.fmradio_music_playing_in_listen);
						mHandler.sendEmptyMessage(MSG_SHUTDOWN);
					}
				} else if (action
					...
					Log.d(TAG, "Got phone state change, new state "
							+ phoneState);
					if (phoneState.equals(TelephonyManager.EXTRA_STATE_IDLE)) {
						mHandler.sendEmptyMessageDelayed(
								MSG_RESTORE_AUDIO_AFTER_CALL, 3000);
					} else {
					...
					if (intent.getIntExtra(
							AudioManager.EXTRA_VOLUME_STREAM_TYPE, -1) == AudioManager.STREAM_MUSIC) {
						int volume = intent.getIntExtra(
								AudioManager.EXTRA_VOLUME_STREAM_VALUE, 0);
						Log.d(TAG,
								"Received FM volume change intent, setting volume to "
										+ volume);
						Preferences
								.setVolume(FMRadioPlayerService.this, volume);
						setFMVolume(volume);
					}
				} else if (mReady
						&& action.equals(SettingsActivity.ACTION_RSSI_UPDATED)) {
					setSeekSensitivity(intent.getIntExtra(
							SettingsActivity.EXTRA_RSSI, -1));
				}
			}
		};
	...
	private void updateStateIndicators() {
		...
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				...
				String rdsName = cursor
						.getString(FMUtil.CHANNEL_COLUMN_RDSNAME);
	...
	private void updateFmStateBroadcast(boolean active) {
		Intent intent = new Intent(
				"com.android.media.intent.action.FM_STATE_CHANGED");
		intent.putExtra("active", active);
		sendStickyBroadcast(intent);
	}

	private boolean isHeadsetConnected() {
		return mHeadsetState == STEREO_HEADSET
				|| mHeadsetState == STEREO_HEADSET2
	...
	private boolean handleHeadsetChange(int state) {
		...
		Log.v(TAG, "Headset change: state " + state + " -> available "
				+ available);
		if (available) {
			if (isMusicPlaying()) {
			...
			Message msg = Message.obtain(mHandler, MSG_SHOW_NOTICE,
					R.string.fmradio_no_headset, 0, null);
			mHandler.sendMessage(msg);
			mHandler.sendEmptyMessage(MSG_SHUTDOWN);
			return false;
		} else {
			Message msg = Message.obtain(mHandler, MSG_SET_ROUTING,
					FM_ROUTING_SPEAKER, 0, null);
			mHandler.sendMessage(msg);
		}
	...
	private void notifyRdsUpdate() {
		if (mCallbacks != null) {
			try {
				mCallbacks.onRdsDataChanged(mCurFreq, mRdsStationName,
						mRdsRadioText, mRdsPTYValue);
			} catch (RemoteException e) {
	...
	private void handleTuneComplete(boolean success, int frequency) {
		Log.v(TAG, "FM tune complete, success " + success + " frequency "
				+ frequency);
		mCurFreq = frequency;
		if (!success) {
			notifyTuneResult(false);
		} else if (!mReady) {
			int lastFreq = Preferences
					.getLastFrequency(FMRadioPlayerService.this);
