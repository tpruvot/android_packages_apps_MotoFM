...
public class FMRadioMain extends ListActivity implements SeekBar.OnSeekBarChangeListener,
    ...
    private IFMRadioPlayerServiceCallbacks.Stub mServiceCallbacks = new IFMRadioPlayerServiceCallbacks.Stub() {
        ...
        public void onSeekFinished(boolean success, int newFrequency) {
            Message msg = Message.obtain(mHandler, MSG_SEEK_FINISHED, newFrequency,
                    success ? 1 : 0, null);
            mHandler.sendMessage(msg);
        }
        ...
        public void onScanFinished(boolean success, int newFrequency) {
            Message msg = Message.obtain(mHandler, MSG_SCAN_FINISHED, newFrequency,
                    success ? 1 : 0, null);
            mHandler.sendMessage(msg);
        }
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (audioRouting == FMRadioPlayerService.FM_ROUTING_HEADSET) {
            menu.add(Menu.NONE, BY_LOUDSPEAKER_ID, Menu.FIRST + 2, R.string.by_loudspeaker)
                    .setIcon(R.drawable.ic_menu_loud_speaker);
        } else {
            MenuItem headsetItem = menu.add(Menu.NONE, BY_HEADSET_ID, Menu.FIRST + 2,
                    R.string.by_headset);
        ...
        switch (requestCode) {
            case CLEAR_CODE:
                if (resultCode == RESULT_OK) {
                    boolean clearedAll = data.getBooleanExtra(FMClearChannel.EXTRA_CLEARED_ALL,
                            false);
                    if (clearedAll) {
        ...
        mChannelList.setOnCreateContextMenuListener(new View.OnCreateContextMenuListener() {
            ...
            public void onCreateContextMenu(ContextMenu menu, View view, ContextMenuInfo menuinfo) {
                ...
                Cursor cursor = getContentResolver()
                        .query(uri, FMUtil.PROJECTION, null, null, null);
                if (cursor != null) {
                    cursor.moveToFirst();
                    int frequency = cursor.getInt(FMUtil.CHANNEL_COLUMN_FREQ);
                    if (frequency == 0) {
                        ...
                        if (mCurFreq != frequency) {
                            ...
                            menu.add(Menu.NONE, REPLACE_MENU_ID, Menu.FIRST + 2,
    ...
    /**
     * bind to FM player service
     *
     * @return true if bound sucessfully
     */
    ...
    private void initiateTune(View v, boolean upward) {
        // disableUIExceptButton(v);
        displayRdsScrollText(false);
        Message msg = Message.obtain(mHandler, MSG_CONTINUE_TUNE, upward ? 1 : 0, 0, null);
        mHandler.sendMessage(msg);
    }
     ...
     *
     * @param freq
     *            Frequency at which seek should start
     * @param upward
    ...
    /**
     * Update frequency displayed in radio panel
     *
     * @param currentFreq
     *            Current frequency
     * @param isEditEnable
     ...
     *
     * @param preset
     *            Position of preset to be selected
     */
     ...
     * Get selected preset
     *
     * @return Position of selected preset
     */
    ...
    /**
     * Find out the position of first empty item
     *
     * @return position/id of first empty item.
     */
    private int getIndexOfEmptyItem() {
        Cursor cursor = getContentResolver().query(Channels.CONTENT_URI, FMUtil.PROJECTION, null,
                null, null);
        int count = 0;
    ...
    /**
     * Find weather dtatbase is empty.
     *
    ...
    /**
     * Shows dialog to save a channel
     *
     * @param position
     *            channel position
     */
    ...
    /**
     * Shows dialog to edit a channel
     *
     * @param position
     *            channel position
     */
    ...
    /**
     * Handles changed rds data.
     */
    private void handleRdsDataChanged() {
        Log.v(TAG, "RDS data changed, station " + mRdsStationName + " radio text " + mRdsRadioText
                + " pty " + mRdsPTYValue);
        Preferences.setLastChannel(this, getSelectedPreset());
    ...
    /**
     * Handles a finished or cancelled scan
     *
     * @param canceled
     *            cancelled or not
     */
    ...
    /**
     * Handles a newly found station while mscanning.
     *
     * @param frequency
     *            Frequency
     */
    private void handleScannedStation(int frequency) {
        Log.d(TAG, "Scanned station on frequency " + frequency + ", scanned so far "
                + mScannedStations);
        mCurFreq = frequency;
    ...
    /**
     * Handles finished or aborted seek.
     *
     * @param aborted
     *            aboorted or not
     */
