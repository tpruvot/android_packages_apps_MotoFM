...
public class FMRadioMain extends Activity implements
	...
	private static final int[] NUMBER_IMAGES = new int[] {
			R.drawable.fm_number_0, R.drawable.fm_number_1,
			R.drawable.fm_number_2, R.drawable.fm_number_3,
			R.drawable.fm_number_4, R.drawable.fm_number_5,
			R.drawable.fm_number_6, R.drawable.fm_number_7,
			R.drawable.fm_number_8, R.drawable.fm_number_9 };
	...
	private static final int[] PTY_STRINGS = new int[] { 0,
			R.string.fm_pty_list_01, R.string.fm_pty_list_02,
			R.string.fm_pty_list_03, R.string.fm_pty_list_04,
			R.string.fm_pty_list_05, R.string.fm_pty_list_06,
			R.string.fm_pty_list_07, R.string.fm_pty_list_08,
			R.string.fm_pty_list_09, R.string.fm_pty_list_10,
			R.string.fm_pty_list_11, R.string.fm_pty_list_12,
			R.string.fm_pty_list_13, R.string.fm_pty_list_14,
			R.string.fm_pty_list_15, R.string.fm_pty_list_16,
			R.string.fm_pty_list_17, R.string.fm_pty_list_18,
			R.string.fm_pty_list_19, R.string.fm_pty_list_20,
			R.string.fm_pty_list_21, R.string.fm_pty_list_22,
			R.string.fm_pty_list_23, R.string.fm_pty_list_24,
			R.string.fm_pty_list_25, R.string.fm_pty_list_26,
			R.string.fm_pty_list_27, R.string.fm_pty_list_28,
			R.string.fm_pty_list_29, R.string.fm_pty_list_30,
			R.string.fm_pty_list_31, 0, R.string.fm_pty_list_33,
			R.string.fm_pty_list_34, R.string.fm_pty_list_35,
			R.string.fm_pty_list_36, R.string.fm_pty_list_37,
			R.string.fm_pty_list_38, R.string.fm_pty_list_39,
			R.string.fm_pty_list_40, R.string.fm_pty_list_41,
			R.string.fm_pty_list_42, R.string.fm_pty_list_43,
			R.string.fm_pty_list_44, R.string.fm_pty_list_45,
			R.string.fm_pty_list_46, R.string.fm_pty_list_47,
			R.string.fm_pty_list_48, R.string.fm_pty_list_49,
			R.string.fm_pty_list_50, R.string.fm_pty_list_51,
			R.string.fm_pty_list_52, R.string.fm_pty_list_53,
			R.string.fm_pty_list_54, R.string.fm_pty_list_55,
			R.string.fm_pty_list_56, R.string.fm_pty_list_57,
			R.string.fm_pty_list_58, R.string.fm_pty_list_59,
			R.string.fm_pty_list_60, R.string.fm_pty_list_61,
			R.string.fm_pty_list_62, R.string.fm_pty_list_63 };
	...
	private class ChannelListAdapter extends ResourceCursorAdapter {
		private class ViewHolder {
			...
			public void bind(Context context, Cursor cursor) {
				...
				boolean selected = cursor.getPosition() == mChannelList
				...
				if (selected && Integer.valueOf(frequency) != 0) {
					...
					mPeakOneAnimation = (AnimationDrawable) mPeakOne
							.getDrawable();
					mPeakTwoAnimation = (AnimationDrawable) mPeakTwo
							.getDrawable();
		...
		public void onServiceConnected(ComponentName className, IBinder service) {
			...
			if (success && mProgressDialog == null) {
				mProgressDialog = ProgressDialog.show(FMRadioMain.this, "",
						getString(R.string.fmradio_waiting_for_power_on), true,
						true);
	...
	private final Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
				...
				if (msg.arg1 != 0) {
					...
					try {
						mAM.setStreamVolume(AudioManager.STREAM_MUSIC,
								Preferences.getVolume(context), 0);
					} catch (Exception e) {
						e.printStackTrace();
					}
			...
			case MSG_AUDIO_MODE_CHANGED:
				...
				mStereoStatus.setVisibility(stereo ? View.VISIBLE
						: View.INVISIBLE);
				break;
			case MSG_ERROR:
				Log.d(TAG, "FM error");
	...
	private IFMRadioPlayerServiceCallbacks.Stub mServiceCallbacks = new IFMRadioPlayerServiceCallbacks.Stub() {
		...
		public void onEnabled(boolean success) {
			Message msg = Message.obtain(mHandler, MSG_POWERON_COMPLETE,
					success ? 1 : 0, 0, null);
		...
		public void onSeekFinished(boolean success, int newFrequency) {
			Message msg = Message.obtain(mHandler, MSG_SEEK_FINISHED,
					newFrequency, success ? 1 : 0, null);
			mHandler.sendMessage(msg);
		}

		@Override
		public void onScanUpdate(int newFrequency) {
			Message msg = Message.obtain(mHandler, MSG_STATION_SCANNED,
					newFrequency, 0, null);
			mHandler.sendMessage(msg);
		}
		...
		public void onScanFinished(boolean success, int newFrequency) {
			Message msg = Message.obtain(mHandler, MSG_SCAN_FINISHED,
					newFrequency, success ? 1 : 0, null);
			mHandler.sendMessage(msg);
		}
		...
		public void onAbortComplete(int newFrequency) {
			Message msg = Message.obtain(mHandler, MSG_SEEK_SCAN_ABORTED,
					newFrequency, 0, null);
			mHandler.sendMessage(msg);
		}
		...
		@Override
		public void onRdsDataChanged(final int frequency,
				final String stationName, final String radioText, final int pty) {
			mHandler.post(new Runnable() {
				@Override
				public void run() {
					boolean valid = frequency == mCurFreq;
					mRdsStationName = valid ? stationName : null;
					mRdsRadioText = valid && radioText != null ? radioText
							.replaceAll("\n", " ") : null;
		...
		public void onAudioModeChanged(boolean stereo) {
			Message msg = Message.obtain(mHandler, MSG_AUDIO_MODE_CHANGED,
					stereo ? 1 : 0, 0, null);
			mHandler.sendMessage(msg);
		}
	};
	...
	protected Dialog onCreateDialog(int id, Bundle args) {
		...
		case DIALOG_SAVE_CHANNEL: {
			...
			return new SaveChannelDialog(this, frequency, preset, name,
					new SaveChannelDialog.OnSaveListener() {
						@Override
		...
		case DIALOG_SCAN_PROGRESS:
			...
			mProgressDialog
					.setTitle(getString(R.string.fmradio_scanning_title));
			mProgressDialog.setMessage(MessageFormat.format(
					getString(R.string.scan_progress), 0));
					...
					.setOnCancelListener(new DialogInterface.OnCancelListener() {
						...
						public void onCancel(DialogInterface dialog) {
							...
							mHandler.sendEmptyMessageDelayed(
									MSG_STOP_SCAN_ANIMATION, SCAN_STOP_DELAY);
						}
					});
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		menu.add(Menu.NONE, EXIT_ID, Menu.FIRST + 5, R.string.exit).setIcon(
		...
		if (audioRouting == FMRadioPlayerService.FM_ROUTING_HEADSET) {
			menu.add(Menu.NONE, BY_LOUDSPEAKER_ID, Menu.FIRST + 2,
					R.string.by_loudspeaker).setIcon(
					R.drawable.ic_menu_loud_speaker);
		} else {
			MenuItem headsetItem = menu.add(Menu.NONE, BY_HEADSET_ID,
					Menu.FIRST + 2, R.string.by_headset);
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		case REPLACE_MENU_ID:
			...
			saveStationToDB(pos, mCurFreq, hasRds ? null : "",
					hasRds ? mRdsStationName : "");
			updateDisplayPanel(mCurFreq, updatePresetSwitcher());
			break;
		case CLEAR_MENU_ID:
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case CLEAR_CODE:
			if (resultCode == RESULT_OK) {
				boolean clearedAll = data.getBooleanExtra(
						FMClearChannel.EXTRA_CLEARED_ALL, false);
				if (clearedAll) {
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case R.id.btn_seekforward:
			...
			mHandler.sendEmptyMessageDelayed(MSG_STOP_SCAN_ANIMATION,
					SCAN_STOP_DELAY);
			break;
		case R.id.btn_reduce:
		case R.id.btn_add:
	...
	private void initListView() {
		...
		mCursor = getContentResolver().query(Channels.CONTENT_URI,
		...
		mChannelList
				.setOnCreateContextMenuListener(new View.OnCreateContextMenuListener() {
					...
					public void onCreateContextMenu(ContextMenu menu,
						...
						int pos = (int) info.id;
						Uri uri = Uri.withAppendedPath(Channels.CONTENT_URI,
								String.valueOf(pos));
						...
						if (cursor != null) {
							...
							int frequency = cursor
									.getInt(FMUtil.CHANNEL_COLUMN_FREQ);
							if (frequency == 0) {
								saveChannel(pos);
							} else {
								menu.setHeaderTitle(FMUtil.getPresetListString(
										FMRadioMain.this, cursor));
								if (mCurFreq != frequency) {
									menu.add(Menu.NONE, PLAY_MENU_ID,
											Menu.FIRST, R.string.play_preset);
									menu.add(Menu.NONE, REPLACE_MENU_ID,
											Menu.FIRST + 2,
											R.string.replace_preset);
								}
								menu.add(Menu.NONE, EDIT_MENU_ID,
										Menu.FIRST + 1, R.string.edit_preset);
								menu.add(Menu.NONE, CLEAR_MENU_ID,
										Menu.FIRST + 3, R.string.clear_preset);
							}
							cursor.close();
						} else {
	...
	private void initSeekBar() {
		...
		mScanAnimation = (AnimationDrawable) getResources().getDrawable(
				R.drawable.fm_progress_red);
	...
	private void initiateTune(View v, boolean upward) {
		// disableUIExceptButton(v);
		displayRdsScrollText(false);
		Message msg = Message.obtain(mHandler, MSG_CONTINUE_TUNE, upward ? 1
				: 0, 0, null);
		mHandler.sendMessage(msg);
	}
	...
	private void playClickPreset(int position) {
		...
		Cursor cursor = getContentResolver().query(uri, FMUtil.PROJECTION,
				null, null, null);
	...
	private void updateFrequencyDisplay(int currentFreq, boolean isEditEnable) {
		if (currentFreq < FMUtil.MIN_FREQUENCY
				|| currentFreq > FMUtil.MAX_FREQUENCY) {
			return;
		}
		...
		Log.v(TAG, "FMRadio updateDisplay: currentFreq " + currentFreq
				+ " -> digits " + digit1 + " " + digit2 + " " + digit3 + " "
				+ digit4);

		int[] numbers = isEditEnable ? NUMBER_IMAGES : NUMBER_IMAGES_UNSELECTED;
		int dot = isEditEnable ? R.drawable.fm_number_point
				: R.drawable.fm_number_unselect_point;

		mFreqDigits[0].setImageResource(numbers[digit1]);
		mFreqDigits[0].setVisibility(digit1 == 0 ? View.INVISIBLE
				: View.VISIBLE);
	...
	private boolean updatePresetSwitcher() {
		...
		if (mCurFreq > 0) {
			Cursor cursor = getContentResolver().query(Channels.CONTENT_URI,
					FMUtil.PROJECTION, Channels.FREQUENCY + "=?",
					new String[] { String.valueOf(mCurFreq) }, null);
	...
	private void handleRdsDataChanged() {
		Log.v(TAG, "RDS data changed, station " + mRdsStationName
				+ " radio text " + mRdsRadioText + " pty " + mRdsPTYValue);
	...
	private void handleScanFinished(boolean canceled) {
		...
		sb.append(MessageFormat.format(getString(R.string.scan_result),
				mScannedStations));

		Toast.makeText(FMRadioMain.this, sb.toString(), Toast.LENGTH_SHORT)
				.show();
	...
	private void handleScannedStation(int frequency) {
		Log.d(TAG, "Scanned station on frequency " + frequency
				+ ", scanned so far " + mScannedStations);
		mCurFreq = frequency;

		Log.d(TAG, "progress dialog " + mProgressDialog + " scanned "
				+ mScannedStations);
		if (mProgressDialog == null) {
			return;
		}
		...
		if (mScannedStations < PRESET_NUM) {
			...
			final String message = MessageFormat.format(
					getString(R.string.scan_progress), mScannedStations);
	...
	private void handleSeekFinished(boolean aborted) {
		...
		if (!aborted) {
			if (mLongPressedButton == R.id.btn_seekbackward) {
				Message msg = Message.obtain(mHandler, MSG_CONTINUE_SEEK, 0, 0,
						null);
				mHandler.sendMessageDelayed(msg, LONG_PRESS_SEEK_TIMEOUT);
			} else if (mLongPressedButton == R.id.btn_seekforward) {
				Message msg = Message.obtain(mHandler, MSG_CONTINUE_SEEK, 1, 0,
						null);
				mHandler.sendMessageDelayed(msg, LONG_PRESS_SEEK_TIMEOUT);
			}
		}
